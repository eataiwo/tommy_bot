  ...
  initialized_(false)
{
  ...
  imu_subscriber_.reset(new ImuSubscriber(
    nh_, ros::names::resolve("sensors/imu") + "/data_raw", queue_size));

  if (use_mag_)
  {
    mag_subscriber_.reset(new MagSubscriber(
      nh_, ros::names::resolve("sensors/imu") + "/mag", queue_size));
...
{
  if (use_mag_)
    ROS_WARN_STREAM("Still waiting for data on topics " << ros::names::resolve("sensors/imu") << "/data_raw"
                    << " and " << ros::names::resolve("sensors/imu") << "/mag" << "...");
  else
    ROS_WARN_STREAM("Still waiting for data on topic " << ros::names::resolve("sensors/imu") << "/data_raw" << "...");
}
